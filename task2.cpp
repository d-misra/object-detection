...
//replication and duplication bug is removed.
...
int main()
{
    ...
    cout << "\n";
    /*
    auto struct_result = generate_random_trainsets(num_of_dtrees,pic_per_class,tot_classes,label_per_feats);
    vector<Mat1f> feat_trainset_per_tree(num_of_dtrees);
    vector<Mat> labels_trainset_per_tree(num_of_dtrees);
    feat_trainset_per_tree=struct_result.feat;
    labels_trainset_per_tree=struct_result.label;
    */
    ...
    for (int i = 0; i < test_class; i++) //looping through all the classes
    {
        ...
        for (int j = 0; j < img_per_test_class; j++)
        {
            ...
            for (int tree_idx = 0; tree_idx < num_of_dtrees; tree_idx++)
            {
                ...
                cout << "\ncurrent class predicted by tree " << tree_idx << " is " << curr_predictd;
                //cout<<"\npredicted class before updating "<< predictd_class[curr_predictd];
                predictd_class[(int)curr_predictd]++;
            ...
            std::cout << "\nmaximum number of times predicted class by forest is  " << max_predicted_class;
